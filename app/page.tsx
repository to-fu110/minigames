      ...
      <Accordion title="設定" colorStyle={getcolorStyles(color).text}>
        <div className="p-4 text-gray-300">
          {["pink", "green", "blue"].map((c) => (
            <label key={c}>
              <input type="radio" name="color" checked={color === c} value={c} onChange={() => setColor(c)} className="w-0 h-0 peer"/>
...
function GameArticles({gamename, href, setumei, color} : {gamename: string, href: string, setumei: string, color: string}) {
  return (
    <Link href={href} className={`border-2 border-gray-500 rounded-md bg-slate-900 font-black ${getcolorStyles(color).text} px-4 py-6 m-4  hover:translate-y-1 shadow-lg hover:bg-slate-800 hover:${getcolorStyles(color).shadow}`}>
        <h2>{gamename}</h2>
        <p>{setumei}</p>
        Go to {gamename}
    </Link>
